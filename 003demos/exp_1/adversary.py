...
mnist = input_data.read_data_sets('./datasets/mnist/gzs/')
x_train = mnist.train.images
y_train = mnist.train.labels

EPSILON = 0.01
SAMPLE_SIZE = 100 # 总数有 9000 个攻击样本
Make_Round = 100
...
def build_image(x_feed, input_class, target_class):
	...
	for output in outputs:
		...
		write_jpeg(out, './datasets/Ad_image/%d_target_input.%d.%d.jpg' % (index, target_class, input_class))
		index += 1
...
def Make_All_Advesarial(input_class):
	...
	with tf.Session() as sess:
		...
		cse_loss = graph.get_tensor_by_name('training/cross_entropy:0')
		acc = graph.get_tensor_by_name('training/accuracy:0')
		...
		for k in range(10):
			...
			for j in range(SAMPLE_SIZE):
				...
				while sess.run(acc, feed_dict={x: np.reshape(x_feed[j], (1, 784)), y: np.reshape(target_class, (1, 10)), keep_prob: 1.0}) != 1.0:
					...
					if i % 100 == 0:
						...
						sys.stdout.flush()
				print('j:',j, ',and k:' k, 'has been constructed.')
